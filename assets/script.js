...
(async()=>{
 ...
 const shown=guests.filter(g=>{const code=im[g.invitation_group]?.rsvp_code||'';const hay=[g.first_name,g.last_name,g.invitation_group,code,g.family_group,g.notes].filter(Boolean).join(' ').toLowerCase();return(!q||hay.includes(q))&&(!tf||g.guest_type===tf)&&(!rf||g.rsvp_status===rf)&&(!sf||g.side===sf)});
 const shownEl=document.getElementById('shownGuestCount');if(shownEl)shownEl.textContent=shown.length;
 body.innerHTML=shown.length?shown.map(g=>{const name=[g.first_name,g.last_name].filter(Boolean).join(' '),notes=[g.side&&`Side: ${g.side}`,g.family_group&&`Group: ${g.family_group}`,g.adult_child,g.dietary_requirements&&`Dietary: ${g.dietary_requirements}`,g.accommodation&&`Accommodation: ${g.accommodation}`,g.transport&&`Transport: ${g.transport}`,g.notes].filter(Boolean).join(' | '),code=im[g.invitation_group]?.rsvp_code||'';return `<tr><td><b>${escapeHtml(name)}</b></td><td>${g.guest_type==='Evening'?'Evening':'Main'}</td><td><select data-rsvp-id="${g.id}"><option ${g.rsvp_status==='Awaiting reply'?'selected':''}>Awaiting reply</option><option ${g.rsvp_status==='Accepted'?'selected':''}>Accepted</option><option ${g.rsvp_status==='Declined'?'selected':''}>Declined</option></select></td><td>
  <b>${escapeHtml(g.invitation_group)}</b>
  ${code?`<br><span class="small">RSVP code</span><br><strong>${escapeHtml(code.toUpperCase())}</strong><br><button class="btn light" style="margin-top:6px" type="button" data-copy-code="${code.toUpperCase()}">Copy code</button>`:''}
</td><td>${escapeHtml(notes)}</td><td><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn light" type="button" data-delete-guest="${g.id}">Remove</button></div></td></tr>`}).join(''):'<tr><td colspan="6">No guests match those filters.</td></tr>'}
...
(async()=>{
 ...
 async function load(){
   ...
   msg.textContent=`${openCount} job${openCount===1?'':'s'} still to do.`;
   const badge=document.getElementById('checklistCountBadge');
...
// ---------------------------
// Private dashboard: Supabase costs + logout
// ---------------------------
(async()=>{
  const costForm=document.getElementById('costForm');
  const costBody=document.querySelector('#costTable tbody');
  if(!costForm || !costBody || !window.weddingSupabase) return;

  const {data:{session}}=await weddingSupabase.auth.getSession();
  if(!session){ location.replace('../login.html'); return; }

  document.getElementById('planningLogout')?.addEventListener('click',async()=>{
    await weddingSupabase.auth.signOut();
    location.replace('../login.html');
  });

  const costMsg=document.getElementById('costSavedMessage');
  const packagePaidInput=document.getElementById('packagePaidAmount');
  const savePackagePaid=document.getElementById('savePackagePaid');
  const BASE_TOTAL=12550;
  let dashboardCosts=[];
  let packagePaid=0;

  function costStatus(cost,paid){
    if(paid<=0) return 'Not paid';
    if(paid>=cost) return 'Paid';
    return 'Part paid';
  }

  function renderDashboardCosts(){
    costBody.innerHTML=dashboardCosts.length
      ? dashboardCosts.map(c=>{
          const cost=Number(c.actual_cost ?? c.estimated_cost ?? 0);
          const paid=Math.max(0,Number(c.paid_amount ?? 0));
          const balance=Math.max(0,cost-paid);
          const status=costStatus(cost,paid);
          return `<tr>
            <td><b>${escapeHtml(c.item)}</b></td>
            <td>${escapeHtml(c.details||'')}</td>
            <td>${money(cost)}</td>
            <td><div style="display:flex;gap:6px;align-items:center;min-width:150px">
              <input type="number" min="0" step="0.01" value="${paid.toFixed(2)}" data-paid-input="${c.id}" style="max-width:105px">
              <button class="btn light" type="button" data-save-paid="${c.id}">Save</button>
            </div></td>
            <td>${money(balance)}</td>
            <td><b>${status}</b></td>
            <td><button class="btn light" type="button" data-dashboard-delete-cost="${c.id}">Remove</button></td>
          </tr>`;
        }).join('')
      : '<tr><td colspan="7" class="small">No extras added yet.</td></tr>';

    const extras=dashboardCosts.reduce((sum,c)=>sum+Number(c.actual_cost ?? c.estimated_cost ?? 0),0);
    const extrasPaid=dashboardCosts.reduce((sum,c)=>sum+Math.max(0,Number(c.paid_amount ?? 0)),0);
    const extrasBalance=Math.max(0,extras-extrasPaid);
    const total=BASE_TOTAL+extras;
    const paidTotal=Math.max(0,packagePaid)+extrasPaid;
    const balanceTotal=Math.max(0,total-paidTotal);

    ['extrasTotal','extrasTableTotal'].forEach(id=>{ const el=document.getElementById(id); if(el) el.textContent=money(extras); });
    const ep=document.getElementById('extrasPaidTotal'); if(ep) ep.textContent=money(extrasPaid);
    const eb=document.getElementById('extrasBalanceTotal'); if(eb) eb.textContent=money(extrasBalance);
    ['weddingGrandTotal','topWeddingTotal'].forEach(id=>{ const el=document.getElementById(id); if(el) el.textContent=money(total); });
    const wp=document.getElementById('weddingPaidTotal'); if(wp) wp.textContent=money(paidTotal);
    const wb=document.getElementById('weddingBalanceTotal'); if(wb) wb.textContent=money(balanceTotal);
    if(packagePaidInput && document.activeElement!==packagePaidInput) packagePaidInput.value=packagePaid.toFixed(2);
  }

  async function loadPackagePaid(){
    const {data,error}=await weddingSupabase.from('wedding_settings').select('setting_value').eq('setting_key','package_paid_amount').maybeSingle();
    if(error){ if(costMsg) costMsg.textContent='Could not load package payment: '+error.message; return; }
    packagePaid=Math.max(0,Number(data?.setting_value ?? 0));
  }

  async function loadDashboardCosts(){
    if(costMsg) costMsg.textContent='Loading costs…';
    await loadPackagePaid();
    const {data,error}=await weddingSupabase.from('wedding_costs').select('*').order('sort_order').order('created_at');
    if(error){ if(costMsg) costMsg.textContent='Could not load costs: '+error.message; return; }
    dashboardCosts=data||[];
    renderDashboardCosts();
    if(costMsg) costMsg.textContent='Costs and payments loaded from Supabase.';
  }

  savePackagePaid?.addEventListener('click',async()=>{
    const amount=Number(packagePaidInput.value);
    if(!Number.isFinite(amount) || amount<0){ if(costMsg) costMsg.textContent='Enter a valid package payment amount.'; return; }
    if(amount>BASE_TOTAL){ if(costMsg) costMsg.textContent='Package paid cannot be more than the £12,550 package total.'; return; }

    if(costMsg) costMsg.textContent='Saving package payment…';
    const {error}=await weddingSupabase.from('wedding_settings').upsert({
      setting_key:'package_paid_amount',
      setting_value:String(amount)
    },{onConflict:'setting_key'});

    if(error){ if(costMsg) costMsg.textContent='Could not save package payment: '+error.message; return; }
    packagePaid=amount;
    renderDashboardCosts();
    if(costMsg) costMsg.textContent='Package payment saved.';
  });
  ...
  costForm.addEventListener('submit',async e=>{
    ...
    const paidAmount=Number(document.getElementById('costPaidAmount').value||0);

    if(!item || !Number.isFinite(amount) || amount<0){ if(costMsg) costMsg.textContent='Enter an item and a valid cost.'; return; }
    if(!Number.isFinite(paidAmount) || paidAmount<0 || paidAmount>amount){ if(costMsg) costMsg.textContent='Paid amount must be between £0 and the item cost.'; return; }

    if(costMsg) costMsg.textContent='Saving…';
    const {error}=await weddingSupabase.from('wedding_costs').insert({
      item,
      ...
      sort_order:dashboardCosts.length+1
    });

    if(error){ if(costMsg) costMsg.textContent='Could not add cost: '+error.message; return; }

    costForm.reset();
    document.getElementById('costPaidAmount').value='0';
    await loadDashboardCosts();
    if(costMsg) costMsg.textContent=`${item} added and saved to Supabase.`;
  });

  costBody.addEventListener('click',async e=>{
    const saveBtn=e.target.closest('[data-save-paid]');
    if(saveBtn){
      const id=saveBtn.dataset.savePaid;
      const input=costBody.querySelector(`[data-paid-input="${id}"]`);
      const row=dashboardCosts.find(c=>c.id===id);
      const cost=Number(row?.actual_cost ?? row?.estimated_cost ?? 0);
      const amount=Number(input?.value);

      if(!Number.isFinite(amount) || amount<0 || amount>cost){ if(costMsg) costMsg.textContent='Paid amount must be between £0 and the item cost.'; return; }

      if(costMsg) costMsg.textContent='Saving payment…';
      const {error}=await weddingSupabase.from('wedding_costs').update({
        paid_amount:amount,
        paid:amount>=cost && cost>0
      }).eq('id',id);

      if(error){ if(costMsg) costMsg.textContent='Could not save payment: '+error.message; return; }
      await loadDashboardCosts();
      if(costMsg) costMsg.textContent='Payment updated.';
      return;
    }

    const btn=e.target.closest('[data-dashboard-delete-cost]');
    if(!btn) return;
    if(!confirm('Remove this cost item?')) return;

    const {error}=await weddingSupabase.from('wedding_costs').delete().eq('id',btn.dataset.dashboardDeleteCost);
    if(error){ if(costMsg) costMsg.textContent='Could not remove cost: '+error.message; return; }

    await loadDashboardCosts();
    if(costMsg) costMsg.textContent='Cost removed.';
  });
