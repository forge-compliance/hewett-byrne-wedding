...
// Kept separate from Guest Manager so moving the guest form to guests.html
// does not disable the dashboard cost tracker.
// ---------------------------
(async()=>{
  ...
  function renderDashboardCosts(){
    costBody.innerHTML=dashboardCosts.length
      ? dashboardCosts.map(c=>`<tr>
          <td><b>${escapeHtml(c.item)}</b></td>
          <td>${escapeHtml(c.details||'')}</td>
          <td>${money(c.actual_cost ?? c.estimated_cost)}</td>
          <td><button class="btn light" type="button" data-dashboard-delete-cost="${c.id}">Remove</button></td>
        </tr>`).join('')
      : '<tr><td colspan="4" class="small">No extras added yet.</td></tr>';

    const extras=dashboardCosts.reduce((sum,c)=>sum+Number(c.actual_cost ?? c.estimated_cost ?? 0),0);
    const total=12550+extras;

    ['extrasTotal','extrasTableTotal'].forEach(id=>{
      const el=document.getElementById(id);
      if(el) el.textContent=money(extras);
    });
    ['weddingGrandTotal','topWeddingTotal'].forEach(id=>{
      const el=document.getElementById(id);
  ...
  async function loadDashboardCosts(){
    ...
    if(error){
      if(costMsg) costMsg.textContent='Could not load costs: '+error.message;
      return;
    }

    dashboardCosts=data||[];
    renderDashboardCosts();
    if(costMsg) costMsg.textContent='Costs loaded from Supabase.';
  }
  ...
  costForm.addEventListener('submit',async e=>{
    ...
    if(!item || !Number.isFinite(amount) || amount < 0){
      if(costMsg) costMsg.textContent='Enter an item and a valid cost.';
      return;
    }
    ...
    if(error){
      if(costMsg) costMsg.textContent='Could not add cost: '+error.message;
      return;
    }
  ...
  costBody.addEventListener('click',async e=>{
    ...
    const {error}=await weddingSupabase
      .from('wedding_costs')
      .delete()
      .eq('id',btn.dataset.dashboardDeleteCost);

    if(error){
      if(costMsg) costMsg.textContent='Could not remove cost: '+error.message;
      return;
    }
